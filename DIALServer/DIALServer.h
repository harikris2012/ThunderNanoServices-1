    ...
    public:
        ...
        public:
            ...
            virtual uint32_t Start(const string& data, const string& payload)
            {
                ...
                if (_passiveMode == true) {
                    const string message(_T("{ \"application\": \"") + _callsign + _T("\", \"request\":\"start\", \"data\":\"") + data + _T("\", \"payload\": \"") + payload + _T("\" }"));
            ...
            {
                if (_passiveMode == true) {
                    const string message(_T("{ \"application\": \"") + _callsign + _T("\", \"request\":\"stop\", \"data\":\"") + data + _T("\", \"payload\": \"") + payload + _T("\" }"));
                    _service->Notify(message);
                    _parent->event_stop(_callsign, data); // TODO: adjust json interface
                } else {
                    if (_switchBoard != nullptr) {
            ...
            bool URL(const string& url, const string& payload) override
            {
                ...
                if (_hasRuntimeChange == true) {
                    if (_passiveMode == true) {
                        const string message(_T("{ \"application\": \"") + _callsign + _T("\", \"request\":\"change\", \"data\":\"") + url + _T("\", \"payload\": \"") + payload + _T("\" }"));
                        _service->Notify(message);
                        result = true;
                    }
                    else {
